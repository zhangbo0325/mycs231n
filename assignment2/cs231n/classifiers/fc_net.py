...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dhidden, dW2, db2 = affine_backward(dout, cache2)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        hidden, cache1, cache2, cache3, cache4, bn, out = {}, {}, {}, {}, {}, {}, {}
        out[0] = X
        for i in range(layer_num-1):
            W, b = self.params['W'+str(i+1)], self.params['b'+str(i+1)]
            if self.use_batchnorm:
                gamma, beta = self.params['gamma' + str(i+1)], self.params['beta' + str(i+1)]        
                hidden[i], cache1[i] = affine_forward(out[i], W, b)        
                bn[i], cache2[i] = batchnorm_forward(hidden[i], gamma, beta, self.bn_params[i])        
        ...
        for i in range(t):
            if self.use_batchnorm:
                ...
                dout[t-i-1], grads['W'+str(t-i)], grads['b'+str(t-i)] = affine_relu_backward(dout[t-i], cache3[t-i-1])
